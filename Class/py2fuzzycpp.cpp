...
OCRCharacters Py2FuzzyCpp::OCRFindText(const QString text, const QString tag)
{
    ...
    try {
        ...
        if (!hello_func || !PyCallable_Check(hello_func)) {
            ...
            std::cerr << __LINE__ << ": 函数 fuzzy_fuzzy() 未找到或不可调用" << std::endl;
        ...
        if (!pValue1 || !pValue2) {
            std::cerr << __LINE__ << ": 创建参数失败" << std::endl;
            Py_XDECREF(pValue1);
            Py_XDECREF(pValue2);
            Py_DECREF(pArgs);
        ...
        if (pValue != nullptr) {
            ...
            PyErr_Print();
            std::cerr << "调用 Python 函数失败" << std::endl;
            isOpen = false;
        }
    }
    catch (const std::exception& e) {
...
OCRCharacter Py2FuzzyCpp::OCR2FindAll(const QString tag, const QStringList data)
{
    ...
    try {
        ...
        if (!pFunc || !PyCallable_Check(pFunc)) {
            if (PyErr_Occurred()) PyErr_Print();
            std::cerr << __LINE__ << ": 函数 differentials() 未找到或不可调用" << std::endl;
            Py_XDECREF(pFunc);
            Py_DECREF(module);
            isOpen = false;
            PyGILState_Release(gstate);
        ...
        for (int i = 0; i < data.size(); ++i) {
            PyObject* pItem = PyUnicode_FromString(data[i].toStdString().c_str());
